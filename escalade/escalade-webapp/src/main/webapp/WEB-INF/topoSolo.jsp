...
<html>
    ...
    <c:import url="/inc/navBar.jsp"/>
		   	...
		   	<ul>
		   		<c:forEach items="${topo.listLocationTopo }" var="locationTopoVar">
			   		<c:if test="${locationTopoVar.accepte == true }">
			   			<li>du ${ locationTopoVar.dateDebutLocationFormat} au ${locationTopoVar.dateFinLocationFormat } </li>
			   		</c:if>
		   		</c:forEach>
		   	</ul>
		</div>
		...
		<div class="container">
			...
			<h3>Dates</h3>
			<input type="hidden" name="dateDebutLocationDemande" value="2018-04-06T08:30:00" />
			<input type="hidden" name="dateFinLocationDemande" value="2018-04-06T18:30:00" />
			<input type="hidden" name="topo_id" value="1" />
				<sec:csrfInput/>
